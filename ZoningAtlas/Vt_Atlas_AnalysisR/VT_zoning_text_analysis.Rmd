...
pacman::p_load(tidyverse, janitor, lubridate, ggthemes, devtools)
...
```{r}
vermont_counties <- urbnmapr::states |> 
  dplyr::filter(state_name == "Vermont")

```

```{r}
#Future plan is to merge vermont_counties with import county summary data, then display with ggplot.
```
...
As of right now this only looks at family sizes of 3 and 4, need pivot longer x5 columns to make a x5 treatment.
